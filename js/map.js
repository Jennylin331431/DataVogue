...
class WorldMap {
  ...
  initVis() {
    ...
    this.svg = d3
      ...
      .attr(
        "viewBox",
        `0 0 ${this.dynamicWidth + 200} ${this.dynamicHeight + 200}`
      )
  ...
  wrangleData() {
    ...
    vis.filteredData = vis.data.filter(
      (d) => d.Year >= vis.startYear && d.Year <= vis.endYear
    );
    ...
    vis.countryDataMap.set(
      "United States of America",
      vis.countryDataMap.get("United States of America") || {
        waste: 0,
        water_usage: 0,
        carbonFootprint: 0,
        count: 0,
    ...
    vis.countryDataMap.set(
      "United Kingdom",
      vis.countryDataMap.get("United Kingdom") || {
        waste: 0,
        water_usage: 0,
        carbonFootprint: 0,
        count: 0,
  ...
  updateLegend(colorScale, minValue, maxValue) {
    ...
    let tickValues = d3
      .range(0, numTicks)
      .map((i) => minValue + (i / (numTicks - 1)) * (maxValue - minValue));
  ...
  updateVis() {
    ...
    Promise.all([
      ...
      .then(([world]) => {
        ...
        let minValue = d3.min(
          [...vis.countryDataMap.values()],
          (d) => d[vis.selectedMetric] / d.count || 0
        );
        let maxValue = d3.max(
          [...vis.countryDataMap.values()],
          (d) => d[vis.selectedMetric] / d.count || 0
        ...
        vis.svg
          ...
          .attr("fill", (d) => {
            ...
            return countryStats
              ? colorScale(
                  countryStats[vis.selectedMetric] / countryStats.count || 0
                )
              : "#ccc";
          })
          .attr("stroke", "#333")
          .on("mouseover", (event, d) => {
            ...
            vis.tooltip.transition().duration(200).style("opacity", 1).style("display", "block");
            ...
            if (stats) {
              ...
              let avgWaterUsage = (
                ...
                stats.count /
                1e6
              ).toFixed(3); // Convert to million liters (X.YYZ format)
              let avgCarbonFootprint = Math.round(
                stats.carbonFootprint / stats.count
              ); // No decimals
              ...
              vis.tooltip.html(
                `<strong style="font-size: 18px;">${countryName}</strong><br>Data Not Collected`
              );
            }
          ...
          .on("mousemove", (event) => {
            ...
            vis.tooltip.style("left", `${posX}px`).style("top", `${posY}px`);
          })
          .on("mouseout", (event) => {
            vis.tooltip.transition().duration(200).style("opacity", 0).style("display", "none");
